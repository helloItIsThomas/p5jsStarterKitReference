...
let gridShapeW, gridShapeH;
let gridCountX, gridCountY;
let gridGutter;

let mouseHoldX, mouseHoldY;
let toggleW, toggleH;
let toggleX, toggleY;
...
let label;
let myCodeStrings;
let codeStringFolderBody = [];
let codeStringFolderHead = [];
...
function preload(){
  ...
  apercuBold = loadFont('data/apercuBold.otf');

  for(let s=0; s<140; s++){
    codeStringFolderBody[s] = loadStrings('textBoxesBody/textBoxCode' + s + '.txt');
    codeStringFolderHead[s] = loadStrings('textBoxesHead/textBoxCodeHead' + s + '.txt');
  }
}

function libSetup(){
  menuString = "show code";
  gridGutter = 10;
  gridCountX = 6;
  gridCountY = 5;

  label = createElement(
    'label', '<input id="input" type="checkbox"/><span class="slider round"></span>'
  );
